...
//        enemyReserve = new RealArmy();
    ...
    // typeArmyHephaestus order to check attack army or defence army is activated hephaestus
    // typeArmyHephaestus is 0 if that army is activated hephaestus
    public void makeArtilleryAttack(BattleField enemy, int typeArmyHephaestus) {
        int i = 0, j = 0;
        ArrayList<UnitFighter> myArtillery = new ArrayList<>();
        for (Slot slot : artillery) {
            for (UnitFighter unit : slot.getUnit()) {
                if (typeArmyHephaestus == 0) {
                    checkHephaestus(unit);
                }
        ...
        if (!isAllDeadInArrayList(enemyFront)) {
            ...
            while (i < myArtillery.size() && !isAllDeadInArrayList(enemyFront)) {
                ...
                float dam = myArtillery.get(i).getDamage();
...
//                            i++;
    ...
    public void makeLongRangeAttack(BattleField enemy, int typeArmyHephaestus) {
        ...
        for (Slot slot : longRangeFighter) {
            for (UnitFighter unit : slot.getUnit()) {
                ...
                System.out.println(unit.getDamage() + "+" + unit.getArmour());
            }
        }
        ...
        if (!isAllDeadInArrayList(enemyFront)) {
            ...
            while (i < myLongRange.size() && !isAllDeadInArrayList(enemyFront)) {
                ...
                float dam = myLongRange.get(i).getDamage();
...
//                            i++;
    ...
    public void makeFrontLineAttack(BattleField enemy, int typeArmyHephaestus) {
        int i = 0, j = 0;
        ...
        if (!isAllDeadInArrayList(enemyFront)) {
            ...
            while (i < myFrontLine.size() && !isAllDeadInArrayList(enemyFront)) {
                ...
                float dam = myFrontLine.get(i).getDamage();
                //if(!(enemyFront.get(j) instanceof Wall)) dam = 80;
                while (dam > enemyFront.get(j).getArmour()) {
                    if (dam > enemyFront.get(j).getArmour() + enemyFront.get(j).getHitPoint()) {
                        dam -= enemyFront.get(j).getArmour() + enemyFront.get(j).getHitPoint();
...
//                            i++;
    ...
    public void makeBombersAttack(BattleField enemy, int typeArmyHephaestus) {
        int i = 0, j = 0;
        ...
        if (!isAllDeadInArrayList(enemyArtillery)) {
            ...
            while (i < myBombers.size() && !isAllDeadInArrayList(enemyArtillery)) {
                ...
                float dam = myBombers.get(i).getDamage();
...
//                            i++;
    ...
    public void makeAirDefenseAttack(BattleField enemy, int typeArmyHephaestus) {
        int i = 0, j = 0;
        ...
        if (!isAllDeadInArrayList(enemyBombers)) {
            ...
            while (i < myAirDefense.size() && !isAllDeadInArrayList(enemyBombers)) {
                ...
                float dam = myAirDefense.get(i).getDamage();
...
//                            i++;
    ...
    public void makeFlanksAttack(BattleField enemy, int typeArmyHephaestus) {
        int i = 0, j = 0;
        ...
        if (!isAllDeadInArrayList(enemyFlank)) {
            ...
            while (i < myFlanks.size() && !isAllDeadInArrayList(enemyFlank)) {
                ...
                float dam = myFlanks.get(i).getDamage();
                while (dam > enemyFlank.get(j).getArmour()) {
                    if (dam > enemyFlank.get(j).getArmour() + enemyFlank.get(j).getHitPoint()) {
                        dam -= enemyFlank.get(j).getArmour() + enemyFlank.get(j).getHitPoint();
                        enemyFlank.get(j).setHitPoint(0);
...
//                            i++;
    ...
    public void checkHephaestus(UnitFighter unit) {
        this.hephaestus = HephaestusUI.hephaestus;
        if (hephaestus != null) {
            long currentTime = System.currentTimeMillis();
            // If in time duration
            if (currentTime <= hephaestus.getTimeDuration()) {
                if (!unit.isActivatedHephaestus()) {
                    unit.setArmour(unit.getArmour() + hephaestus.getAddArmor());
                    unit.setDamage(unit.getDamage() * (1 + hephaestus.getAddDamage()));
                    unit.setActivatedHephaestus(true);
                }
            } else {
                if (unit.isActivatedHephaestus()) {
                    unit.setArmour(unit.getArmour() - hephaestus.getAddArmor());
                    unit.setDamage(unit.getDamage() / (1 + hephaestus.getAddDamage()));
                    unit.setActivatedHephaestus(false);
                }
            }
        }
    }
